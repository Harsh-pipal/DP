...
ll find(ll i, ll j, string &s, string &p, vector<vector<ll>>&dp){
    ...
    if(s[i]==p[j]){
        return dp[i][j] = find(i-1,j-1,s,p,dp) + find(i-1,j,s,p,dp);
    }
    else{
        return dp[i][j] = find(i-1,j,s,p,dp);
    }
}
...
void solve(){
    ...
    //number of occurrences of P in string S
