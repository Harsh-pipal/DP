...
ll find(ll ind, ll w, vector<ll>&wt,vector<ll>&value,vector<vector<ll>>&dp){
    if(ind==0){   
        ll temp = w/wt[ind];
        return temp*value[ind];   
    }
    if(dp[ind][w]!=-1)
        return dp[ind][w];
    //no take 
    ll notake = 0 + find(ind-1,w,wt,value,dp);

    //take
    ll take = -1;
    if(w>=wt[ind])
        take = value[ind] + find(ind,w-wt[ind],wt,value,dp);
...
void solve(){
    ...
    vll wt(n),value(n);
    ll sum=0;
    ///only difference between unbounded and bounded is that we dont move to previous index while taking case
    for(int i=0;i<n;i++){
        cin>>wt[i];
    }
    for(int i=0;i<n;i++){
    ...
    //cout<<find(n-1,w,wt,value,dp)<<nl;
